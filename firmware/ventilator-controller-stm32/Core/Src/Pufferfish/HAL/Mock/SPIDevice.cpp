...
#include <Pufferfish/HAL/Mock/SPIDevice.h>
...
void SPIDevice::set_read(const uint8_t *buf, size_t count) {
  size_t index = 0;
  size_t minumum = (count < read_buf_size) ? count : read_buf_size;
...
void SPIDevice::get_write(uint8_t *buf, size_t &count) {
  size_t index = 0;
...
void SPIDevice::set_write_read(const uint8_t *buf, size_t count) {
  size_t index = 0;
  size_t minumum = (count < read_buf_size) ? count : read_buf_size;
...
void SPIDevice::get_write_read(uint8_t *buf, size_t count) {
  size_t index = 0;
...
bool SPIDevice::get_chip_select() const {
  return last_cs_;
}

void SPIDevice::set_return_status(SPIDeviceStatus input) {
